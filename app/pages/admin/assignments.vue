  ...
  <div>
    <div class="flex items-center justify-between mb-8">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-0.5">Associez des coachs à leurs clients.</p>
      </div>
    </div>

    <!-- Create form -->
    <div class="card mb-6">
      <h2 class="font-semibold text-gray-800 mb-4">Nouvelle assignation</h2>
      ...
      <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-100">
          <tr>
            ...
            <th class="px-4 py-3 text-left font-medium text-gray-600">Assigné le</th>
...
<script setup lang="ts">
  ...
  async function submitCreate() {
    ...
    try {
      ...
      form.coachId = ''
      form.clientId = ''
      showToast('Assignation créée avec succès.', 'success')
      await refresh()
    } catch (e) {
  ...
  async function removeAssignment(a: Assignment) {
    ...
    try {
      ...
      const err = e as { data?: { statusMessage?: string }; statusMessage?: string }
      showToast(err?.data?.statusMessage ?? 'Erreur lors de la suppression.', 'error')
    } finally {
      removing.value = null
    }
  }
