...
vi.mock('h3', () => {
  return {
    getHeader: (event: any, name: string) => event.headers[name.toLowerCase()] || event.headers[name],
    readBody: async (event: any) => JSON.parse(event.raw),
    useRawBody: async (event: any) => (Buffer.from(event.raw)),
...
describe('payments webhook integration test (mocked h3)', () => {
  ...
  it('accepts signed webhook and processes payment', async () => {
    const envelope = { event: 'payment.succeeded', data: { id: 'pay_abc', status: 'paid', reference: 'order1', amount: 2000, currency: 'XOF' } }
    const raw = JSON.stringify(envelope)

    // compute signature the same way verifyWebhookSignature does
    const sig = crypto.createHmac('sha256', process.env.KKIAPAY_WEBHOOK_SECRET!).update(raw).digest('hex')
