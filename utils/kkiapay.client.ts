interface KkiapaySDK {
  open: (opts: { token: string }) => void
}

let sdk: KkiapaySDK | null = null

export default async function useKkiapay(): Promise<KkiapaySDK> {
  if (sdk) return sdk

  // Load official Kkiapay SDK script if not already present.
  // The real SDK URL should be verified from Kkiapay docs.
  const src = process.client
    ? process.env.KKIAPAY_SDK_URL || 'https://js.kkiapay.me/v1/kkiapay.js'
    : null
  if (src && document) {
    ...
    const win = window as unknown as { Kkiapay?: KkiapaySDK }
    sdk = win.Kkiapay ?? {
